var talkShowList;
var talentList;
var musicMixTitleList;
var preview = new MobilePreviewForm('mobilepreview_slider_container');

//Auto complete stuff
function setupAutoCompleteForMix(update) {

	if (update) {
		$( "#mix_title").typeahead().data('typeahead').source = musicMixTitleList;
		$( "#new_mix_title").typeahead().data('typeahead').source = musicMixTitleList;
	} else {
		$( "#mix_title" ).typeahead({
			source: musicMixTitleList
		});
		$( "#new_mix_title" ).typeahead({
			source: musicMixTitleList
		});
	}
}
function updateAutoCompleteForMix() {

	$.ajax(
		{
			url: "/content/musicMixTitleList",
			type: "get",
			dataType: "json",
			success: function (resp) {
				if (resp.code === 0 && resp.data) {
					musicMixTitleList = resp.data;
					setupAutoCompleteForMix(true);
				}
			}
		}
	).fail(function () {
...
function createMix () {
	...
	$.ajax (
		...
		if (resp.code === 0) {
			...
			$('#who').val(content.who);
			$('#mix_title').val(content.mix_title);
			$('#content_id').val(content.id);

			//Update preview
			preview.renderPreviewInfo('content', content.id, null, 'musicmix');
...
$(document).ready(function() {
	...
	var calendar = $('#calendar').fullCalendar({
		...
		events: {
			url: '/content/getMusicMixes/',
			type: 'get',
			cache: true,
			error: function() {
				$('.saveProgress').show().html('Error. Could not load events. Please try reloading').css('color', 'red');
				setTimeout(function() { $('.saveProgress').fadeOut(); }, 6000);
			}
		},
		...
		eventResize: function(event, delta, revertFunc, jsEvent) {
			...
			preview.renderPreviewInfo('content', event.id, null, 'musicmix');
...
//                $('#calendar').fullCalendar('select', date.start, date.end);
	...
	$('#content_content_type_id').on("change", function() {
		...
		if (contentType == 0 || contentType == ContentTypeIDOfMusicMix) return;
